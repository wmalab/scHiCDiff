...
#' (scHi-C) data by assuming assuming the read counts follows negative binomial
#' (NB) distribution. 
#' 
...
#' mu_1, mu_2, theta_1, theta_2: MLE of the negative binomial distribution's parameters of
#'  group 1 and group 2, where mu and theta represent the mean and dispersion estimate of
...
scHiCDiff.NB <- function(count.table, group){
  library(pscl)
  library(gamlss)
  library(VGAM)
  library(maxLik)
