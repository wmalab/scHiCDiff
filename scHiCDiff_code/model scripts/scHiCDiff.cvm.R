scHiCDiff.CVM <- function(count.table,group){
  library(twosamples)
  ...
  callDCI.CVM <- function(i){
    ...
    res.binpair <- cvm_test(counts_1,counts_2)
  ...
  results <- matrix(data-NA, nrow=binpair,ncol=5, dimnames=list(row.names(counts),c("bin1","bin2","stat","pvalue","pvalue.adj")))
  results <- as.data.frame(results)
  for(i in 1:binpairNum){
    cat("\r",paste0("scHiCDiff.CVM is analyzing",i,"of",binpairNum,"bin paris"))
    results[i,1:2] <- binpair[i,]
    results[i.3:5] <- callDCI.CVM(i)
  }
